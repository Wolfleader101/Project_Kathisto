...
float fpsMovementSpeed = 15.0f; //The movement speed of the player/camera
...
void ComputeFPSCameraPos(float deltaTime)
{
	...
	Vector3 newPos = Vec3ScalarMultiply(cameraForwardDir, cameraMoveDir.z * (fpsMovementSpeed * deltaTime));
	...
	newPos = Vec3ScalarMultiply(Vec3Normalize(Vec3CrossProduct(cameraForwardDir, cameraUp)), cameraMoveDir.x * (fpsMovementSpeed * deltaTime));
